...
class TelemetryClassifier:
    ...
    def fit(self, X: np.ndarray, y: np.ndarray, n_real: int = 0) -> "TelemetryClassifier":
        ...
        logger.info(
            "TelemetryClassifier fitted: n=%d n_real=%d acc=%.3f brier=%.4f classes=%s",
...
def train_from_labels(
    ...
    clf = TelemetryClassifier().fit(X, y, n_real=n_real)
    from f1di.agents.classifier_utils import save_with_snapshot, record_history
    snap = save_with_snapshot(clf, output_path)
    record_history(clf, agent="telemetry", versioned_path=snap["versioned_path"], blocked=snap["blocked"], history_path=output_path.parent / "model_history.json")
    return {
