...
class BatteryClassifier:
    def __init__(self, model_type: str = DEFAULT_MODEL_TYPE) -> None:
        ...
        self._scaler, self._model = build_model(model_type, max_depth=4, agent="battery")
    ...
    def fit(self, X: np.ndarray, y: np.ndarray, n_real: int = 0, sample_weight: np.ndarray | None = None) -> "BatteryClassifier":
        ...
        logger.info("BatteryClassifier fitted: n=%d n_real=%d cv_acc=%.3f cv_brier=%.4f n_splits=%d", self.n_train, self.n_real, self.accuracy, self.brier_score, self.cv_n_splits)
        return self

    @staticmethod
    def _build_pipeline(model_type: str = DEFAULT_MODEL_TYPE):
        from f1di.agents.classifier_utils import build_model
        return build_model(model_type, max_depth=4, agent="battery")
...
def generate_synthetic(n: int = 600, seed: int = 42) -> tuple[np.ndarray, np.ndarray]:
    ...
    while len(X) < n:
        ...
        X.append([soc, slope, speed, phase, laps_r, stint, circuit_speed, circuit_type, race_laps])
...
def train_from_labels(
    ...
    blend = blend_with_transfer(
        lambda: BatteryClassifier._build_pipeline(model_type),
    ...
    clf = BatteryClassifier(model_type=model_type).fit(X, y, n_real=n_real, sample_weight=sample_weight)
    ...
    record_history(clf, agent="battery", versioned_path=snap["versioned_path"], blocked=snap["blocked"], history_path=output_path.parent / "model_history.json", threshold=snap.get("threshold"))
...
_INCREMENTAL_PATH = Path("data/calibration/battery_incremental.pkl")
